...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// Write victim page to swap, if needed, and update pagetable
        	...
        	// Obtain a pointer to victim's pte
		pgtbl_entry_t *victim = coremap[frame].pte;
                
                // Swap out victim page if it has been modified
		if(victim->frame & PG_DIRTY) {
                        // Write victim to the swap file
		...
		// Update status bits for victim page
		victim->frame = (victim->frame & ~PG_VALID) | PG_ONSWAP;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgdir_entry_t *cur_entry = &pgdir[idx];
	// Initialize a page table if it has not been done yet
	if(!cur_entry->pde) 
		*cur_entry = init_second_level(); 

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	idx = PGTBL_INDEX(vaddr);

	pgtbl_entry_t *pgtbl = (pgtbl_entry_t *)(cur_entry->pde & PAGE_MASK);	 
	p = &pgtbl[idx];

	// Check if p is valid or not, on swap or not, and handle appropriately

	// Note that PG_MISS was added to pagetable.h

	// Check if access to p is hit
	if(p->frame & PG_VALID) {
		p->frame = p->frame & ~PG_MISS;
		hit_count++;

	// Check if access to p is miss
	} else {
		// Find frame for p in physical memory first
		int frame = allocate_frame(p);

		// If p is swapped out, bring it back to physical memory
		if(p->frame & PG_ONSWAP) {
			swap_pagein(frame, p->swap_off);
			// Update frame number and status bits for p
			p->frame = (p->frame & PGTBL_MASK) | (frame << PAGE_SHIFT);
			p->frame = (p->frame & ~PG_DIRTY & ~PG_ONSWAP) | PG_VALID | PG_MISS;
		}

		// If p is accessed for the first time, initialize frame for p 
		else {
			init_frame(frame, vaddr);
			// Update frame number and status bits for p
			p->frame = (p->frame & PGTBL_MASK) | (frame << PAGE_SHIFT);
			p->frame = p->frame | PG_VALID | PG_DIRTY | PG_MISS;					
	...
	// Check access type and update status bits accordingly
	if(type == 'M' || type == 'S')
