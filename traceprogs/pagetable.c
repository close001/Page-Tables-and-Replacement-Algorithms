...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t *victim = coremap[frame].pte;
                ...
                // Swap out victim if it is dirty
		if(victim->frame & PG_DIRTY) {
                        // Write victim to the swap file (do we need to check for return value?)
			victim->swap_off = swap_pageout(frame, victim->swap_off);
			evict_dirty_count++;
		}
		else evict_clean_count++;

		// Update frame for victim
		victim->frame = (victim->frame & ~PG_VALID & ~PG_DIRTY) | PG_ONSWAP;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// Assuming that mem_align sets last 12 bits to 0
	...
	if(p->frame & PG_VALID) {		
		...
		if(p->frame & PG_ONSWAP) {
			...
			p->frame = (p->frame & ~PG_DIRTY & ~PG_ONSWAP) | PG_VALID;
		}
		...
		// If p is accessed for the first time, initialize frame for p 
			...
			// Update frame number and status bits for p
			p->frame = (p->frame & PGTBL_MASK) | (frame << PAGE_SHIFT);
			p->frame = p->frame | PG_VALID | PG_DIRTY | PG_REF;					
